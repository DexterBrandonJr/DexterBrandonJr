...
```sh
python3 hub_local.py selftest                          # SELFTEST OK · 23 checks passed
psql "$DATABASE_URL" -f hub-kit.sql                    # into an empty database or project
psql "$DATABASE_URL" -c "select hub.selftest();"      # SELFTEST OK · 23 checks passed
psql "$DATABASE_URL" -c "select * from hub.audit();"  # no rows
sha256sum hub-kit.sql hub_local.py                     # must match the fingerprints in HUB-KIT.md
```
