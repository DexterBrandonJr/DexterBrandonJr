...
- **If the skill being tested writes real files, commits, or pushes rather than just answering in text, redirect that to a scratch location before you spawn anything.** A subagent given a realistic-sounding prompt to check triggering will, if the skill fires, faithfully carry out that skill's real side effects too — including a commit and push to whatever branch is checked out. Testing "does this skill trigger" against a skill whose job is to persist things is not a read-only experiment by default; point it at a temp directory (or otherwise sandbox the target) or you'll find synthetic test content sitting in real shared state.
