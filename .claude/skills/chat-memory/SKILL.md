...
**Some things don't belong in git at all — not even a private repo.** "Private" only narrows who can see the repo right now; it says nothing about permanence. Git history is effectively forever — deleting a file in a later commit leaves the old value fully recoverable from history unless someone does a destructive rewrite, which essentially never happens for a memory store people are actively adding to. For identity-critical secrets — SSNs, full financial account or card numbers, passwords, private keys, anything that alone enables fraud or identity theft — that permanence makes *any* git repo the wrong home, private or public. Don't save these to `chat-memory` at all, even on a private repo, even if asked to. Say plainly that this category doesn't belong in version control and ask where the user actually wants it kept (a password manager, an encrypted note, or whatever mechanism they already use for that class of data) — the "public vs. private" check above and this one are two different questions, and passing the first doesn't clear the second.
